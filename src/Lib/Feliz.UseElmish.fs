...
module Resumable =
    ...
    module Program =
        ...
        let toResumable
            ...
            let map (model, cmd) = model, Cmd.map UserMsg cmd

            let mapInit userInit args =
                match initialState with
                | Some model -> model, Cmd.none
                | None -> userInit args |> map

            let mapUpdate userUpdate msg model =
                match msg with
                | UserMsg userMsg -> userUpdate userMsg model
                | SetState newModel -> newModel, Cmd.none
                |> map
            ...
            program |> Program.map mapInit mapUpdate mapView mapSetState subs
...
type React with
    ...
    static member inline useElmish
        (
            program: unit -> Program<unit, 'Model, 'Msg, unit>,
            ?dependencies: obj array
        ) =
